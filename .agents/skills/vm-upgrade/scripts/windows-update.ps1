...
#
# The whole body is one `& { … }` block, and the file ends with a blank line.
# Both are load-bearing — see "Every multi-line block" in SKILL.md §6. Fed to
# `-Command -` as bare statements, this script would run each line as its own
# top-level command: `trap` would never fire, a failed COM search would leave
# $result null, and `$null.Updates.Count -eq 0` would print "no updates
# available" and exit 0. It did exactly that on 2026-08-28. One block makes the
# trap and the exit code real.
...
& {
  ...
  # ResultCode 2 is orcSucceeded. Anything else means the search did not answer,
  # and "0 updates" from it is not a fact — say so rather than report a clean run.
  if ($result.ResultCode -ne 2) {
    Say ('search did not succeed (ResultCode=' + $result.ResultCode + ') - nothing was installed')
    exit 1
  }
